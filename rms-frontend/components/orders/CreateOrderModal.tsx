...
import { Plus, Minus, Trash2, Search, ShoppingCart, UtensilsCrossed } from 'lucide-react';
...
export function CreateOrderModal({ isOpen, onClose }: CreateOrderModalProps) {
  ...
  // Always fetch — don't pass available:true as a boolean param to avoid
  // string/boolean coercion issues in query params. Filter client-side instead.
  ...
  // Filter client-side: only available items, matching search + category
          ...
          <div className="flex flex-col gap-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="font-semibold text-[#4E342E]">
                ...
                {allMenuItems.length > 0 && (
                  <span className="ml-2 text-xs font-normal text-[#8D6E63]">
            ...
            <div className="border border-[#F5E6D3] rounded-xl overflow-hidden">
              {menuLoading ? (
                ...
                <div className="p-6 text-center text-[#8D6E63] text-sm">
                  {allMenuItems.length === 0
                    ? 'No menu items found. Check your connection.'
                  ...
                  {filteredItems.map((item) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center justify-between px-4 py-3 hover:bg-[#FFF8F0] transition-colors text-left ${
                          inCart ? 'bg-orange-50' : 'bg-white'
                        }`}
                      >
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="border border-[#F5E6D3] rounded-xl overflow-hidden flex-1">
              {cart.length === 0 ? (
                ...
                <div className="max-h-72 overflow-y-auto divide-y divide-[#F5E6D3]">
                  {cart.map((c) => (
                    <div key={c.menuItem.id} className="flex items-center gap-2 px-3 py-2.5 bg-white">
                      <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 flex-shrink-0">
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); updateQuantity(c.menuItem.id, -1); }}
                        ...
                        <span className="w-6 text-center text-sm font-bold text-[#4E342E]">
                          {c.quantity}
                        </span>
                        <button
                          type="button"
                        ...
                        <button
                          type="button"
                          onClick={(e) => { e.stopPropagation(); removeFromCart(c.menuItem.id); }}
            ...
            <div className="bg-[#FFF8F0] rounded-xl border border-[#F5E6D3] p-4">
              ...
              {/* Validation hints */}
              {!tableId && (
                <p className="text-xs text-orange-500 mb-2">⚠ Select a table to continue</p>
              )}
              {cart.length === 0 && (
                <p className="text-xs text-orange-500 mb-2">⚠ Add at least one item</p>
              )}
