"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.UpdateTableStatusDto = exports.UpdateTableDto = exports.CreateTableDto = void 0;
const class_validator_1 = require("class-validator");
const swagger_1 = require("@nestjs/swagger");
const client_1 = require("@prisma/client");
class CreateTableDto {
    tableNumber;
    capacity;
    status;
}
exports.CreateTableDto = CreateTableDto;
__decorate([
    (0, swagger_1.ApiProperty)({ example: 1 }),
    (0, class_validator_1.IsNumber)(),
    (0, class_validator_1.Min)(1),
    __metadata("design:type", Number)
], CreateTableDto.prototype, "tableNumber", void 0);
__decorate([
    (0, swagger_1.ApiProperty)({ example: 4 }),
    (0, class_validator_1.IsNumber)(),
    (0, class_validator_1.Min)(1),
    __metadata("design:type", Number)
], CreateTableDto.prototype, "capacity", void 0);
__decorate([
    (0, swagger_1.ApiPropertyOptional)({ enum: client_1.TableStatus, default: client_1.TableStatus.AVAILABLE }),
    (0, class_validator_1.IsEnum)(client_1.TableStatus),
    (0, class_validator_1.IsOptional)(),
    __metadata("design:type", String)
], CreateTableDto.prototype, "status", void 0);
class UpdateTableDto extends (0, swagger_1.PartialType)(CreateTableDto) {
}
exports.UpdateTableDto = UpdateTableDto;
class UpdateTableStatusDto {
    status;
}
exports.UpdateTableStatusDto = UpdateTableStatusDto;
__decorate([
    (0, swagger_1.ApiProperty)({ enum: client_1.TableStatus }),
    (0, class_validator_1.IsEnum)(client_1.TableStatus),
    __metadata("design:type", String)
], UpdateTableStatusDto.prototype, "status", void 0);
//# sourceMappingURL=table.dto.js.map